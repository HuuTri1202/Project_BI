import type { AdminUserDto, TenantRole } from '@bi/shared';
import type { ResultSetHeader, RowDataPacket } from 'mysql2';
import type { Db } from './db';

/**
 * Truy vấn "người dùng CỦA một tổ chức" — file riêng, đúng như chỉ thị trong
...
/** Trạng thái quyền đọc từ DB, dùng cho `requireFreshAdmin`. */
...
/**
 * Đọc lại quyền THẬT của người gọi trong tổ chức đang mở.
 *
 ...
 * `u.deleted_at`. Một tài khoản đã bị khoá toàn hệ thống mà còn membership admin
 * sẽ lọt qua nó.
 *
...
export async function findAdminContext(
  ...
  const [rows] = await db.query<AdminContextRow[]>(
    `SELECT m.role, m.is_active AS member_active, u.is_active AS user_active
       FROM memberships m
...
/**
 * Escape ký tự đặc biệt của LIKE trước khi bọc `%`.
 *
 * Không escape thì tìm "100%" thành `LIKE '%100%%'` — khớp MỌI dòng; "a_b" thì
 * `_` khớp một ký tự bất kỳ; và một dấu `\` lẻ làm hỏng cả pattern. Người dùng
 * gõ ba ký tự này hoàn toàn bình thường (email có `_` là chuyện thường ngày).
 */
